...
const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: {
    ...
    body: 'Este link não corresponde a um convite válido. Confira a URL ou peça um novo convite à pessoa que o enviou.',
  },
  used: {
  ...
  expired: {
    ...
    body: 'Este convite expirou. Peça ao administrador da conta que gere um novo link.',
  },
  server_error: {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              'Você já está em outra conta. Entre com outro e-mail para participar desta conta.'
          );
        } else {
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            ...
            <div className="text-muted-foreground space-y-2 py-2 text-xs">
              <p>
                ...
                , saia e cadastre-se novamente com outro endereço de e-mail. O
                link do convite continuará válido enquanto não expirar.
              </p>
            </div>
