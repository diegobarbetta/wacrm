    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error(
            'Nenhum agente configurado — conclua a configuração primeiro.'
          );
        } else {
        ...
        {turns.length === 0 && (
          <div className="text-muted-foreground flex h-full flex-col items-center justify-center text-center text-sm">
            ...
            <p className="mt-1 text-xs">
              Ele usa sua base de conhecimento e se comporta como a resposta
              automática, inclusive no encaminhamento.
            </p>
            {onGoToSetup && (
              <Button
                variant="link"
              ...
              >
                Ainda não configurou? Acesse Configuração{' '}
            ...
            >
              ...
              {t.role === 'assistant' && t.handoff && (
                ...
                >
                  ...
                  Encaminharia para uma pessoa neste ponto
                </p>
              )}
            </div>
            {t.role === 'user' && (
